...
type GroupedBy<T, K> = K extends [infer K0, ...infer KR] ? Map<T[Extract<K0, keyof T>], GroupedBy<T, KR>> : T[];
...
export declare function getOrSetFromMapWithEnabled<T>(params: {
    shouldCache: boolean;
    map: Map<string, T>;
    key: string;
    value: () => T | Promise<T>;
}): Promise<T>;
export declare function getOrSetFromMap<T>(params: {
    map: Map<string, T>;
    key: string;
    value: () => T | Promise<T>;
}): Promise<T>;
export declare function normalizeIndent<T extends string>(template: TemplateStringsArray, ...args: T[]): T;
export {};
